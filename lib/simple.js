...
const sms = async(sock, m) => {
	...
	m.replyImg = (buffer, option = { id: m.chat, quoted: m }) => sock.sendMessage(option.id ? option.id : m.chat, {
		image: buffer
	}, {
		quoted: option.quoted ? option.quoted : m
	})
	m.replyVid = (buffer, option = { id: m.chat, quoted: m }) => sock.sendMessage(option.id ? option.id : m.chat, {
		video: buffer
	}, {
		quoted: option.quoted ? option.quoted : m
	})
