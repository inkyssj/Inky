...
const sms = async(sock, m) => {
	...
	m.replyImg = (buffer, option = { id: m.chat, caption: '', mentions: sock.parseMention(caption), quoted: m }) => sock.sendMessage(option.id ? option.id : m.chat, {
		image: buffer,
		caption: option.caption ? option.caption : ''
	}, {
		quoted: option.quoted ? option.quoted : m
	})
	return m
